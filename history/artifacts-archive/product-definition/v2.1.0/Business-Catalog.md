...
|----|-------|-----------|-------|
| `BR-02b` | Single account per verified email | Users signing in with different providers but the same verified email resolve to the same Auth user (`auth.users.id`) where Supabase identity linking supports it; no duplicate app profiles or memberships. | Core |
