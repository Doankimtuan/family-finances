...
# Transaction List (`money.transactions`)
...
money.transaction-detail; money.transaction-add; money.hub
...
`Search`, `ActivityFeed`, `TransactionRow`, `EmptyState`, `TopAppBar`
...
`screen_view:money.transactions` · `action:primary` on primary CTAs · no PII in payloads
