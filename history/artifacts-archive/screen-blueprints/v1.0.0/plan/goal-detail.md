---
screen_id: plan.goal-detail
title: Goal Detail
nav_group: plan
feature_ids: ["F-Plan"]
module: plan
phase: MVP
route: /(product)/plan/goals/[id]
...
`GoalCard`, `Progress`, `Amount`, `Button`
...
`screen_view:plan.goal-detail` · `action:primary` on primary CTAs · no PII in payloads
