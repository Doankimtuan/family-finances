...
Authenticate existing user (OAuth-first; email available)
...
Returning (Daily Partner / Household Steward per Product personas as applicable)

## Entry Points

auth.welcome; Gates unauth; auth.forgot-password; auth.register

## Exit Points

onboard.wizard; home.index; together.members; auth.confirm
...
Follow hierarchy order above; one primary question; calm adult voice; glossary terms. Guest CTA forbidden.

## Primary Actions

Continue with Google, Continue with Apple, Sign in (email)
...
`Input`, `Button`, `Alert`, `Text`, `Divider`
...
`screen_view:auth.login` · `action:primary` on primary CTAs · no PII in payloads
