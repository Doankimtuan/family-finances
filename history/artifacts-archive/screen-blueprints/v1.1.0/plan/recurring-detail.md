---
screen_id: plan.recurring-detail
title: Recurring Detail
nav_group: plan
feature_ids: ["F-Plan"]
module: plan
phase: MVP
route: /(product)/plan/recurring/[id]
...
# Recurring Detail (`plan.recurring-detail`)
...
`Input`, `Amount`, `DatePicker`, `Button`, `Dialog`
...
`screen_view:plan.recurring-detail` · `action:primary` on primary CTAs · no PII in payloads
