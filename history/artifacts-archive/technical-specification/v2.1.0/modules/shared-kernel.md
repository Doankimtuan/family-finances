---
document: Module Spec — shared-kernel
...
# Module Spec — shared-kernel

## Purpose

Cross-cutting primitives with no infrastructure or UI dependencies.
...
- `modules/shared-kernel/application/index.ts` exports commands/queries only.
...
`MoneyAmount`, `HouseholdId`, `UserId`, `RequestId`

## Repositories

`shared-kernelRepository` ports in application; Supabase adapters in infrastructure.
...
`modules/shared-kernel/{errors,ids,money,result,zod}`

## Naming Rules

Per Architecture Naming Standards — `shared-kernel` prefix not required in symbols; folder owns namespace.
