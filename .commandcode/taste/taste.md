...
- Flags fundamental product assumptions that require human input as "PRODUCT DECISION REQUIRED" and refuses to silently decide them — but still delivers a recommended choice, alternative, and tradeoff for each flagged decision rather than using it as an excuse to avoid recommending. Confidence: 0.9
- Prefers decisiveness in architecture/product recommendations: alternatives may be compared, but exactly ONE target model must be chosen — never multiple equally weighted options. Confidence: 0.8
