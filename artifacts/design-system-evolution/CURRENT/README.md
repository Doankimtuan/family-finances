# Design System Evolution - Current

Phase D defines the canonical visual system and UI implementation mindset for ViNha.

This package is governed by:

- Current information architecture in `artifacts/information-architecture/CURRENT/`
- Phase B information architecture in `artifacts/information-architecture/CURRENT/`
- Phase C UX redesign in `artifacts/ux-redesign/CURRENT/`
- Frozen product and business contracts in `artifacts/current/specification/`
- Current shared UI, shared patterns, HeroUI configuration, and semantic theme tokens

Phase D does not change business behavior, information architecture, navigation ownership, domain ownership, financial rules, or UX flow contracts.

## Canonical Direction

ViNha should feel like a Gen Z financial life companion for young Vietnamese households: calm, warm, mobile-native, flexible, and trustworthy. It should make financial facts easier to understand without becoming an accounting tool, bank back office, gamified app, crypto product, or marketing showcase.

## Deliverables

- [executive-summary.md](./executive-summary.md)
- [visual-direction.md](./visual-direction.md)
- [product-personality.md](./product-personality.md)
- [design-principles.md](./design-principles.md)
- [visual-foundation.md](./visual-foundation.md)
- [color-system.md](./color-system.md)
- [typography-system.md](./typography-system.md)
- [spacing-and-layout.md](./spacing-and-layout.md)
- [app-shell.md](./app-shell.md)
- [shape-and-elevation.md](./shape-and-elevation.md)
- [component-taxonomy.md](./component-taxonomy.md)
- [component-contracts.md](./component-contracts.md)
- [page-patterns.md](./page-patterns.md)
- [motion-system.md](./motion-system.md)
- [iconography.md](./iconography.md)
- [illustration-guidelines.md](./illustration-guidelines.md)
- [data-visualization.md](./data-visualization.md)
- [dark-mode-guidelines.md](./dark-mode-guidelines.md)
- [accessibility-guidelines.md](./accessibility-guidelines.md)
- [component-governance.md](./component-governance.md)
- [codex-ui-guide.md](./codex-ui-guide.md)
- [phase-e-input.md](./phase-e-input.md)
- [consistency-report.md](./consistency-report.md)
- [final-verdict.md](./final-verdict.md)

## Implementation Boundary

Future UI work must implement one screen or one coherent flow at a time. Before editing UI, read Phase B, Phase C, this Phase D package, existing shared components, and the target screen in a real browser.
