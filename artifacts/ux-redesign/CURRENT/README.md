# UX Redesign - Current Canonical Contract

Phase C defines behavior and interaction flows for the redesigned ViNha experience.

Inputs used:

- `artifacts/information-architecture/CURRENT/`
- `artifacts/information-architecture/CURRENT/`
- `artifacts/current/domains/`
- `artifacts/current/product/`
- `artifacts/current/specification/`

This phase does not change business behavior, financial invariants, domain ownership, five-tab navigation, route ownership, or visual design.

## Deliverables

- [executive-summary.md](./executive-summary.md)
- [ux-principles.md](./ux-principles.md)
- [critical-journeys.md](./critical-journeys.md)
- [flow-catalog.md](./flow-catalog.md)
- [form-strategy.md](./form-strategy.md)
- [financial-confirmation-model.md](./financial-confirmation-model.md)
- [screen-state-model.md](./screen-state-model.md)
- [mobile-interaction.md](./mobile-interaction.md)
- [cross-module-continuity.md](./cross-module-continuity.md)
- [ux-content-guidelines.md](./ux-content-guidelines.md)
- [accessibility-contract.md](./accessibility-contract.md)
- [friction-analysis.md](./friction-analysis.md)
- [priority-matrix.md](./priority-matrix.md)
- [phase-d-input.md](./phase-d-input.md)
- [consistency-report.md](./consistency-report.md)
- [final-verdict.md](./final-verdict.md)

## Canonical UX Posture

ViNha should feel like a calm household money operating system: quick for daily capture, clear about real money versus intention, protective before consequential actions, and never advisory where the approved contracts forbid advice.
