...
| Installment review | Explain original purchase, card origin, installment schedule/status, paid/remaining amount, and linked records without creating a Loan. | View linked purchase or current billing period. | Return to card detail. | Use the canonical loan presentation or a route-local `ScheduleRow` list when card origin must remain explicit. | Card detail or transaction detail. | `DERIVED_DISPLAY` |
