...
Brand moment while session resolves
...
Route to welcome/login or home gate
...
auth.welcome; auth.login; home.index; onboard.wizard
...
`screen_view:auth.splash` · `action:primary` on primary CTAs · no PII in payloads
