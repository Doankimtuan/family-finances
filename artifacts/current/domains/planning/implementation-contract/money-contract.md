...
| Update Jar Or Allocation | None | None | None | Update virtual allocation/capacity | Home/Health may read updated plan | Review state never blocks; invalid target, invalid amount |
...
| Review Period | None | None | None | May mark viewed or marked_reviewed | Reads facts for context | Skipped review |
...
## Plan V2 Review Contract

- Monthly Review is optional, non-blocking, and report-only.
- A Review state must never be used as a gate for Plan mutations.
- Assisted is a recommendation layer only; it cannot create ledger writes or automatic allocation.
- Historical V1 ritual fields may remain stored, but they are not financial truth and do not control availability.
