...
5. **OAuth + linking (E02)** — Google/Apple OAuth-first Login; BR-02b account linking; no guest (`ST-E02-004`…`005`).
6. **Account lifecycle (E02 P1)** — Sign-out + delete account (`ST-E02-006`).
7. **i18n** — All new user-facing strings in `messages/{en,vi}`; `auth.json` filled from blueprint copy.
8. **Quality** — Each story passes gates before the next starts; Sprint DoD satisfied.
