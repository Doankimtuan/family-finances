...
async function assertActionSheet(page: Page) {
  const body = page.locator('[data-slot="action-sheet-body"]:visible').last();
  const footer = page
    .locator('[data-slot="action-sheet-footer"]:visible')
    .last();
...
test.describe("Plan action sheets", () => {
  test.beforeAll(() => {
    test.skip(
      !process.env.E2E_USER_EMAIL || !process.env.E2E_USER_PASSWORD,
      "E2E credentials not provided",
    );
    execFileSync("node", ["scripts/plan-valuation-fixture.mjs", "setup"], {
      stdio: "inherit",
    });
  });

  test.afterAll(() => {
    execFileSync("node", ["scripts/plan-valuation-fixture.mjs", "cleanup"], {
      stdio: "inherit",
    });
  });
  ...
  test("440px EN/dark: Jar and Goal edit/action sheets", async ({ page }) => {
    ...
    await goal.click();
    await page.getByTestId("goal-more-actions").click();
    ...
    await page.getByTestId("goal-more-actions").click();
    const link = page.getByTestId("goal-link-open").filter({ visible: true });
    if (await link.count()) {
      await link.click();
      await assertActionSheet(page);
      await page.keyboard.press("Escape");
