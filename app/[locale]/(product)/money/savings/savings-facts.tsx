...
/**
 * Grouped definition list for Savings detail. Presentation only: callers
 * still own which facts exist and how values are formatted.
 */
export function SavingsFactsCard({
  title,
  testId,
  footer,
  children,
}: SavingsFactsCardProps) {
  return (
    <section className="flex flex-col gap-(--space-2)" data-testid={testId}>
      {title ? <SavingsSectionTitle>{title}</SavingsSectionTitle> : null}
...
export function SavingsFactRow({
  label,
  value,
  emphasis = false,
  className,
