...
`artifacts/` (soft), `validation/`, `reports/`, `scores/`, `knowledge/` (soft), `specifications/` (soft), `pipelines/`, `architecture-v2/` (soft), `workers/`, `templates/`

## Produces

`reviews/extensibility-reviewer/` → `review-finding` (`folder_mirror`: `reviews/extensibility-reviewer/…`)
...
4. Write under `reviews/extensibility-reviewer/` with `folder_mirror`.
...
- `folder_mirror` under `reviews/extensibility-reviewer/`
