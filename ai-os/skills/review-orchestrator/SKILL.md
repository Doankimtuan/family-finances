...
`artifacts/` (soft), `validation/`, `reports/`, `scores/`, `knowledge/` (soft), `specifications/` (soft), `pipelines/`, `reviews/`

## Produces

`reviews/` → `review-status`; `governance/` → `review-scores` (dual emit; see sample-secondary-payload.json)
...
| merge | Glob `reviews/*/`; cite paths in evidence |
