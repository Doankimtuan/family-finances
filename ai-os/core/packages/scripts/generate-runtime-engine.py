...
WORKERS = [
    ...
    },
    {
        ...
        "mission": "Before execution, generate execution graph, worker order, estimated outputs/runtime, artifacts/validation/review/checkpoint plans. Never execute immediately.",
        "entry_kinds": [
    ...
    },
    {
        "id": "retry-engine",
        "title": "Retry Engine",
        "mission": "Plan retries for failed core/packages/workers/phases/validation/review with updated context and retry limits from runtime config.",
        "entry_kinds": [
...
5. Ten command entry_kinds covered in catalog; sample covers primary set.""",
    "dependency-resolver": """1. Soft-read `core/packages/pipelines/*/dependency-graph.json` and registry workers.
...
3. Discover core/packages/workers/validators/reviewers via registry soft-reads.
...
def gen_entry(w: dict, kind: str, idx: int, **extra) -> dict:
    ...
    e = {
        ...
        "confidence": 0.85,
        "source_paths": ["core/packages/workers/", "core/packages/pipelines/", "core/packages/schemas/", "core/packages/configs/", "core/packages/registry/"],
...
| `artifacts/execution/` | Planner, pipeline engine, artifact/retry managers |
| `commands/` | Command interpreter |
| `state/` | Context + progress |
| `logs/` | Structured logs |
| `events/` | Event bus |
| `core/packages/configs/` | Runtime configuration files |
| `checkpoint/` | Checkpoint manager |
| `resume/` | Resume engine |
| `scheduler/` | Scheduler + dependency resolver |
| `core/packages/templates/` | Report templates |
...
output: ./artifacts
""")
    # also copy default to core/packages/configs/ for consume path convenience
...
def create_gates() -> None:
    ...
    write(ROOT / "reviewers" / "runtime-engine-coverage-review" / "manifest.json", json.dumps({
        ...
        "rubric_ref": "core/packages/reviewers/runtime-engine-coverage-review/rubric/runtime-engine-coverage.json",
    }, indent=2) + "\n")
...
def create_pipeline() -> None:
    ...
    write(pdir / "pipeline.json", json.dumps({
        ...
        "extensions": {
            "schema": "core/packages/schemas/runtime-engine-payload.schema.json",
            "contract": "core/packages/contracts/runtime-engine.md",
            "output_types": [
                "runtime-status", "runtime-event", "runtime-log", "runtime-command",
                "runtime-plan", "runtime-checkpoint", "runtime-execution-report",
                "gate-runtime-report",
...
def update_registries() -> None:
    ...
    validators["entries"]["runtime-engine-schema-check"] = {
        "id": "runtime-engine-schema-check",
        "path": "core/packages/validators/runtime-engine-schema-check",
        "version": "0.1.0",
        "status": "draft",
        "description": "Deterministic checks for runtime-engine payloads.",
    }
    reviewers["entries"]["runtime-engine-coverage-review"] = {
        "id": "runtime-engine-coverage-review",
        "path": "core/packages/reviewers/runtime-engine-coverage-review",
        "version": "0.1.0",
        "status": "draft",
