...
`artifacts/validation/pipeline-validator/` → `validation-finding` (required `folder_mirror`: `artifacts/validation/pipeline-validator/…`)
...
3. Emit entries with full fields + `folder_mirror` under `artifacts/validation/pipeline-validator/`.
...
- Every entry has full fields + `folder_mirror` matching `artifacts/validation/pipeline-validator/`
