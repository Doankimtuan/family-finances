# Worker Conformance Checklist — `discover-contract-inventory`
...
- [x] Directory name equals `discover-contract-inventory` (kebab-case)
- [x] Contains exactly the mandatory files from `core/packages/templates/worker/README.md`
- [x] No undeclared executables that mutate the product repo
- [x] `manifest.json` present and validates against `core/packages/schemas/worker-manifest.schema.json`
...
- [x] Registered in `core/packages/registry/workers.json` and `core/packages/pipelines/discovery/`
...
- [x] Reserved discovery skill package exists at `skills/discover-contract-inventory/`
...
- [x] Reserved validator package exists at `core/packages/validators/discovery-schema-check/`
...
- [x] Reserved reviewer package exists at `core/packages/reviewers/discovery-coverage-review/`

## Output schema

- [x] `output-schema.json` is valid JSON Schema draft 2020-12
- [x] Primary worker envelope present; discovery payload schema is `core/packages/schemas/discovery-report.schema.json`
- [x] Schema `$id` uses core/packages/workers/discover-contract-inventory/output.schema.json pattern
...
- [x] Conforms to `core/packages/contracts/worker-port.md` for Discovery phase
