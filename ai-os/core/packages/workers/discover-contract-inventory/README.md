...
> **Discovery Worker package** `discover-contract-inventory` (class=`discovery`).  
> Instantiated from `core/packages/templates/worker/`. Not a Feature Worker.  
...
| Field | Value |
|-------|-------|
| Worker id | `discover-contract-inventory` |
| Version | `0.1.0` |
| Status | `draft` |
...
Catalog AIOS JSON schemas, contracts, policies, and Zod mirrors; flag reinlined enums and missing $refs without changing product features.
...
- Do not skip core/packages/validators/reviewers declared on the task.

## Contracts this package must honor

1. `core/packages/contracts/worker-port.md`
2. `docs/architecture/execution-lifecycle.md`
3. `docs/architecture/TRACEABILITY.md`
4. `docs/architecture/CONCURRENCY.md` (locks / claim exclusivity)
5. `docs/architecture/SECURITY.md` (side-effect ceiling)
...
| `manifest.json` | Worker port manifest (`core/packages/schemas/worker-manifest.schema.json`) |
...
2. Registered in `core/packages/registry/workers.json` and `core/packages/pipelines/discovery/`.
3. Fixtures under `examples/` and `testcases/` use discovery ids.
4. `manifest.json` validates `core/packages/schemas/worker-manifest.schema.json`.
