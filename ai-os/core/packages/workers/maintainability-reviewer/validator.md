# Validator Binding — `maintainability-reviewer`

> Template for how this worker runs **deterministic** validators.  
> Validators are not skills. They produce `gate-review-report` artifacts only.
...
| Source | Rule |
|--------|------|
| Task `validators` | Run each id if present in `core/packages/registry/validators.json` |
| Worker allow-list | Optional: `review-engine-schema-check` — `{{allowed_validator_ids_or_star}}` |
...
| Run id | yes |
| Validator package (`core/packages/validators/{{validator_id}}`) | yes |
...
2. For each required validator:
   ...
   - Write `gate-review-report` artifact (`core/packages/schemas/validator-report.schema.json`).
...
Every gate-review-report MUST include:
...
Follow `docs/architecture/validator-conventions.md` and the active gate profile `blocking_severities`.
...
- `docs/architecture/validator-conventions.md`
- `core/packages/roles/validator.md`
- `core/packages/templates/validation/report.json`
- `core/packages/schemas/validator-report.schema.json`
- `core/packages/schemas/validator-spec.schema.json`
