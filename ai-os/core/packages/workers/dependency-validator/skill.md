...
`artifacts/validation/dependency-validator/` → `validation-finding` (required `folder_mirror`: `artifacts/validation/dependency-validator/…`)
...
3. Emit entries with full fields + `folder_mirror` under `artifacts/validation/dependency-validator/`.
...
- Every entry has full fields + `folder_mirror` matching `artifacts/validation/dependency-validator/`
