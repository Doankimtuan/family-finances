# Validator Binding — `specification-generator`
...
| Worker allow-list | Optional: `specification-engineering-schema-check` |
...
| Run id | yes |
| Validator package (`core/packages/validators/specification-engineering-schema-check`) | yes |
...
2. For each required validator:
   ...
   - Write `validation-report` artifact (`core/packages/schemas/validator-report.schema.json`).
