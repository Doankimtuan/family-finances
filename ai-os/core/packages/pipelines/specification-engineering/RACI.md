...
1. `artifacts/tasks/` work-item graph is the source of truth for engineering dependencies.
2. `artifacts/roadmap/` sequences releases/phases; it must not invent new product scope or contradict task `depends_on`.
3. `artifacts/implementation/` derives build order from `artifacts/tasks/` + `artifacts/roadmap/` + `artifacts/architecture-v2/` / `artifacts/redesign/`; it must not redesign modules.

## Naming note

Produce folder `artifacts/tasks/` holds **engineering-task-graph** mirrors. It is **not** the Core Engine `task` artifact type used by the planner.
