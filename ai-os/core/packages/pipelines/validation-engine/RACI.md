...
> Resolves overlap between specialized validators writing into `artifacts/validation/`.
...
1. Each finding writer writes under `artifacts/validation/<worker_id>/` (`folder_mirror` required).
2. `validation-orchestrator` merges `artifacts/validation/*/…` findings + `artifacts/scores/` into `validation-status` (does not rewrite finding files).
3. `validation-reporter` reads merged status + scores → `artifacts/reports/` as `validation-report`.
4. Gate packages under `core/packages/validators/` emit **`gate-validation-report`** — not worker `validation-report`.
