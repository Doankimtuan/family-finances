...
5. Register in `core/packages/registry/workers.json` and the appropriate `core/packages/pipelines/<id>/`.
6. Pass every item in `checklist.md` before promoting to `active`.

## Phase rule

- **Discovery Workers** (`worker_class: discovery`) may be instantiated under `ai-os/workers/discover-*`, Product RE (`core/packages/pipelines/product-re/`), or Solution Architecture (`core/packages/pipelines/solution-architecture/`) in the current phase.
