...
**Runtime payloads** use `entries[]` per `core/packages/schemas/specification-engineering-payload.schema.json`.  
...
| postconditions[] | |
| workflow | |
| business_rules | Restate `artifacts/business/` — do not invent |
| validation_rules | |
| error_handling | |
| edge_cases | |
| dependencies[] | |
| acceptance_criteria[] | Pointers to `artifacts/acceptance/` |
...
2. functional
3. non-functional
4. architecture → `artifacts/architecture-v2/`, `governance/decision-records/`
5. module → `artifacts/folder-structure/`
6. feature
7. api
8. database
9. ui
10. security
11. deployment → `artifacts/migration/`, `artifacts/tech-stack/`
12. coding-standards → `artifacts/tech-stack/`
13. testing-strategy
