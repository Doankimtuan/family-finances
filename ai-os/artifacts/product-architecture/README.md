# `artifacts/product-architecture/` — Product reverse-engineering (consume)
...
Produced by `product-architecture-observer`. Consumed by `product-analyst` and (narrowly) `workflow-analyzer`. See `core/packages/pipelines/product-re/`.
