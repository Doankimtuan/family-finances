# Reviewer Binding — `maintainability-reviewer`
...
| Source | Rule |
|--------|------|
| Task `reviewers` | Run each id if present in `registry/reviewers.json` |
| Worker allow-list | Optional: `review-engine-coverage-review` — `{{allowed_reviewer_ids_or_star}}` |
...
| Prior gate-review-report refs | recommended |
...
2. For each required reviewer:
   ...
   - Write `review-report` artifact (`schemas/review-report.schema.json`).
...
- `architecture/reviewer-conventions.md`
- `roles/reviewer.md`
- `templates/review/report.json`
- `schemas/review-report.schema.json`
- `schemas/reviewer-spec.schema.json`
