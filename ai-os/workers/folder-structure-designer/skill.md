# Skill Binding — `folder-structure-designer`

Skill claim/execute binding for Solution Architecture worker `folder-structure-designer`.
Skills live under `skills/folder-structure-designer`. Validators/reviewers are not skills.
...
| Source | Rule |
|--------|------|
| Task `skill_id` | Required; must exist in `registry/skills.json` |
| Task `skill_version` | Exact semver or `"active"` |
| Worker allow-list | Optional: `folder-structure-designer` |
