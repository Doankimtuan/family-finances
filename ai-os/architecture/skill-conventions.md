...
Only. Deterministic checks → `validators/`. Judgment → `reviewers/`.

## Required package files

| File | Required | Schema / template |
|------|----------|-------------------|
| `manifest.json` | yes | `schemas/skill-manifest.schema.json` |
| `SKILL.md` | yes | `templates/skill/SKILL.md` |
| `input.schema.json` | recommended | JSON Schema |
| `output.schema.json` | recommended | JSON Schema |

## Manifest rules

1. `id` unique key in `registry/skills.json` `entries`.
