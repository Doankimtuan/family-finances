...
`templates/`, `schemas/`, `workers/`, `validators/`, `reviewers/`, `pipelines/` (alias `pipeline/`), `knowledge/`, `configs/`, `artifacts/`, `registry/`

## Produces

`framework-generator/generators/{g["id"]}/` → `framework-generation`

## Configuration-driven generation

Accept capability specs as YAML or JSON under `configs/` or inline `generation-spec`. Emit scaffold plans only — do not mutate existing workers.
...
`framework-generator/generators/{g["id"]}/` → `framework-generation` (required `folder_mirror`: `framework-generator/generators/{g["id"]}/…`)
...
1. Load capability spec (YAML/JSON) from `configs/` or task input.
2. Resolve template packs from `templates/` and reference schemas from `schemas/`.
3. Plan outputs under `framework-generator/generators/{g["id"]}/` — scaffold only.
...
1. Payload validates against `schemas/framework-generator-payload.schema.json`.
2. `folder_mirror` starts with `framework-generator/generators/{g["id"]}/`.
...
- [ ] Registered in registry/workers.json and registry/skills.json
- [ ] Partition `framework-generator/generators/{g["id"]}/` documented
...
def gen_worker_manifest(g: dict) -> dict:
    return {
        ...
        "extensions": {
            ...
            "partition": f"framework-generator/generators/{g['id']}/",
        },
    }
...
def gen_sample_payload(g: dict) -> dict:
    entry = {
        ...
        "target": f"framework-generator/generators/{g['id']}/",
        "spec_ref": "configs/sample-capability.yaml",
        "output_plan": {
            "artifacts": [f"framework-generator/generators/{g['id']}/README.md"],
            "registries": ["workers", "skills", "schemas"],
            "semver": "0.1.0",
        },
        "evidence": [f"templates/worker/", f"schemas/worker-manifest.schema.json"],
        ...
        "alternative_solution": "Manual copy from templates/worker/",
        "confidence": 0.85,
        "source_paths": ["configs/sample-capability.yaml", "templates/worker/"],
        "unknowns": ["UNKNOWN: execution phase not open"],
        "traceability": {"spec_version": "0.1.0", "generator": g["id"]},
        "folder_mirror": f"framework-generator/generators/{g['id']}/{g['entry_kinds'][0]}.json",
    }
    return {
        "schema_version": "1.0.0",
...
def create_worker_package(g: dict) -> None:
    ...
    write(wdir / "examples/sample-input.json", json.dumps({"capability": g["title"], "spec": "configs/sample-capability.yaml"}, indent=2) + "\n")
...
{chr(10).join(f'  - {m}' for m in g['modes'])}
outputs:
  partition: framework-generator/generators/{wid}/
registries:
  - workers
  - skills
  - schemas
...
| `templates/` | Generator-specific template overrides |
...
""")
    ...
    write(fg / "templates/README.md", "Generator template overrides; canonical templates remain under `ai-os/templates/`.")
    write(fg / "registries/README.md", "Self-registration plans target registry/*.json — applied only in execution phase.")
    write(
    ...
    write(
        ...
        json.dumps(
            {"schema_version": "1.0.0", "updated_at": NOW, "templates": ["templates/worker/", "templates/skill/", "templates/artifact/"]},
            indent=2,
        )
        + "\n",
    )
  ...
  "primary_output_type": "framework-generation",
        ...
        "goal": {
            ...
            "success_criteria": [
                ...
                {"id": "partitioned", "description": "Outputs under framework-generator/generators/<id>/"},
        ...
        "extensions": {
            "schema": "schemas/framework-generator-payload.schema.json",
            "contract": "contracts/framework-generator.md",
        ...
        json.dumps(
            {
                ...
                "checks": [
                    ...
                    {"check_id": "folder-mirror-partition", "severity": "critical", "rule": "folder_mirror starts with framework-generator/generators/<worker_id>/"},
        ...
        json.dumps(
            {
                ...
                "rubric_ref": "reviewers/framework-generator-coverage-review/rubric/framework-generator-coverage.json",
            },
            indent=2,
        )
        + "\n",
...
3. **Configuration-driven** — capability specs as YAML/JSON under `configs/` or task input.
4. **Partitioned output** — `framework-generator/generators/<worker_id>/` via `folder_mirror`.
...
`framework-generator/` (generators, bootstrap, scaffolding, templates, registries, catalog)
...
def update_registries() -> None:
    ...
    for g in GENERATORS:
        ...
        workers["entries"][wid] = {
            ...
            "consumes": COMMON_CONSUMES,
            "produces": [f"framework-generator/generators/{wid}/"],
        }
        skills["entries"][wid] = {
            "id": wid,
            "path": f"skills/{wid}",
    ...
    validators["entries"]["framework-generator-schema-check"] = {
        "id": "framework-generator-schema-check",
        "path": "validators/framework-generator-schema-check",
        "version": VERSION,
        "status": "draft",
        "description": "Deterministic checks for framework-generator payloads.",
    }
    reviewers["entries"]["framework-generator-coverage-review"] = {
        "id": "framework-generator-coverage-review",
        "path": "reviewers/framework-generator-coverage-review",
        "version": VERSION,
        "status": "draft",
