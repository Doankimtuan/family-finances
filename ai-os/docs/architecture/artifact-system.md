...
| `type` | Kebab id that **must exist** in `core/packages/registry/artifact-types.json` `entries` |
...
1. Add `entries.<type>` to `core/packages/registry/artifact-types.json`
2. Optionally add `core/packages/schemas/<type>.schema.json` for the payload
...
1. `meta.json` — `core/packages/schemas/artifact.schema.json`
