...
For each task: skill pin, inputs, core/packages/validators/reviewers, side-effect budget, `allow_parallel_attempts` / `continue_on_cancel` as needed.
...
3. Plan artifact with `task_refs` + `dependency_graph_ref` (no inlined artifacts/tasks/edges)
