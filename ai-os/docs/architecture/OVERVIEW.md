...
Workers are contracted (`core/packages/contracts/worker-port.md`). **Discovery**, **Product RE**, **Solution Architecture**, and **Specification Engineering** packages use `worker_class: discovery`. **Feature Workers are not implemented**. Core Engine **loads and validates** pipelines; it does not claim/run workers.
...
| Policy | `governance/policies/gate-profiles.json` |
...
9. Product architecture inputs use `artifacts/product-architecture/`, never AIOS `docs/architecture/`.

## Cursor skills vs AIOS skills

See `core/packages/contracts/cursor-bridge.md`.
...
- Breaking changes: `docs/architecture/MIGRATIONS.md`
