...
**Scope:** `core/packages/pipelines/specification-engineering/` and its core/packages/workers/skills/schemas/examples only  
...
| C4 / H8 | Declared consumes: `governance/decision-records/`, `artifacts/tech-stack/`, `artifacts/migration/`, `artifacts/folder-structure/` |
| H1 | [RACI.md](core/packages/pipelines/specification-engineering/RACI.md) for milestone / depends_on / delivery-order / build-order |
| H2 | `artifacts/repository/` ownership = human/pre-step soft input |
...
| H6 | Rubric on disk: `core/packages/reviewers/specification-engineering-coverage-review/rubric/` |
| H7 | Produce-folder `examples/` + specialized TEMPLATEs |

## Medium / clarity

- Pipeline consumes list no longer dual-lists bare `architecture`
- `artifacts/tasks/` folder documented as engineering-task-graph mirrors (≠ Core `task`)
