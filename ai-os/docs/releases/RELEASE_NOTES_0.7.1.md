...
**Scope:** `core/packages/pipelines/validation-engine/` and its core/packages/workers/skills/schemas/examples only  
...
| C2 | Findings partitioned under `artifacts/validation/<worker_id>/` with required `folder_mirror`; orchestrator merge contract |
...
| H1 | [RACI.md](core/packages/pipelines/validation-engine/RACI.md) for overlapping validators |
...
- Distinct worker `*-validator` vs gate `core/packages/validators/*-schema-check`
...
- SA `governance/quality/TEMPLATE.md` untouched; scorecards remain under `governance/quality/validation-scorecard/`
