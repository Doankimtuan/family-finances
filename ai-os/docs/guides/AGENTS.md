# AIOS Agent Guide (v0.13.0)

Canonical agent contract after the structural domain migration. Top-level [`AGENTS.md`](../../AGENTS.md) is a stub pointing here.

**Capability-first:** prefer `docs/capabilities/` and `@Run` over individual workers.
...
| Engine (TS) | `core/` (`knowledge/`, Zod `schemas/`, …) |
| Runtime | `runtime/` |
| Skills | `skills/` |
| Artifacts | `artifacts/` |
...
|-------------|----------------|
| Maintain Discovery Workers (`core/packages/workers/discover-*`) | Implement **Feature Workers** |
| Maintain Product RE Workers (`core/packages/pipelines/product-re/`) | Skill bodies that mutate the product repo |
| Maintain Solution Architecture Workers (`core/packages/pipelines/solution-architecture/`) | Invent business logic / change business rules |
| Maintain Specification Engineering Workers (`core/packages/pipelines/specification-engineering/`) | Redesign the system / invent requirements |
| Maintain Validation Engine Workers (`core/packages/pipelines/validation-engine/`) | Mutate source artifacts / invent missing info / execute validations in packaging milestone |
| Maintain Review Engine Workers (`core/packages/pipelines/review-engine/`) | Mutate sources / regenerate outputs / validate schemas / execute reviews in packaging milestone |
| Maintain Framework Generator Workers (`core/packages/pipelines/framework-generator/`) | Execute generation / mutate prior workers / create project-specific workers in packaging milestone |
| Maintain Qualification Framework Workers (`core/packages/pipelines/qualification-framework/`) | Execute benchmarks / modify framework / regenerate workers / invent ground truth in packaging milestone |
| Maintain Runtime Engine Workers (`core/packages/pipelines/runtime-engine/`) | Execute workers / analyze projects / mutate workers-validators-reviewers / bypass Execution Planner in packaging milestone |
| Extend reserved skills/validators/reviewers for registered pipelines | Bypass artifacts/validation/review gates |
...
| Clarify role charters | Treat `docs/architecture/` as product architecture input |
...
- `meta.json` (`core/packages/schemas/artifact.schema.json`)
...
- Product architecture observations: `artifacts/product-architecture/` (logical consume name `docs/architecture/` for Solution Architecture).
- Never treat AIOS `docs/architecture/` (OVERVIEW / TRACEABILITY / CONCURRENCY) as product architecture input.
- Solution Architecture produces under `redesign/`, `architecture-v2/`, `tech-stack/`, `migration/`, `folder-structure/`, `decision-records/`.

## Framework version

Current: see `VERSION`. Schema `$id` base: `https://aios.dev/schemas/`. Breaking contract changes require `docs/architecture/MIGRATIONS.md` entry.
