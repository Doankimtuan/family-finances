...
**Scope:** `pipelines/product-re/` only (Solution Architecture unchanged)  
...
- `gaps/` working folder + gap worker `produces: ["gaps"]`
- Produce-folder `examples/` for product/workflow/requirements/acceptance/gaps
- Specialized knowledge/features/business `template.schema.json`
- `soft_discovery_handoff` only on ingest workers
- Hard graph edges: workflow←features/architecture ingest; gap←all ingest+transforms (21 edges)
