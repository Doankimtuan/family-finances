-- Fix create_household_invitation: RETURNS TABLE output vars shadow column
-- names in RETURNING (Postgres 42702 ambiguous "token" / "expires_at").
-- Also keep expired-pending cleanup so stale invites do not block re-invite.
