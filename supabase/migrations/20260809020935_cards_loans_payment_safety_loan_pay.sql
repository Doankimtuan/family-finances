...
begin
  ...
  insert into public.transactions (
    ...
    v_loan.household_id, p_account_id, 'liability_payment', v_amount, v_loan.currency,
    v_paid_at, null, null, null, 'posted', v_user_id
  ) returning id into v_tx_id;

  insert into public.loan_payments (
    household_id, loan_id, account_id, transaction_id, amount, principal_paid, interest_paid, paid_at, created_by
  ) values (
    v_loan.household_id, p_loan_id, p_account_id, v_tx_id, v_amount, v_principal, v_interest, v_paid_at, v_user_id
  ...
  if v_completed then
    insert into public.inbox_items (
      household_id, kind, status, source_type, source_id, amount, currency, title, context_json
    ) values (
      v_loan.household_id, 'emi_complete', 'pending', 'guided', p_loan_id, v_loan.monthly_payment,
      v_loan.currency, v_loan.name,
      jsonb_build_object('flow', 'loan_complete', 'review_item_type', 'InstallmentComplete', 'loan_id', p_loan_id, 'principal', v_loan.principal)
    )
    on conflict (household_id, source_type, source_id) do update
      set status = 'pending', updated_at = timezone('utc', now()), title = excluded.title, context_json = excluded.context_json
    returning id into v_item_id;

    if v_item_id is null then
      select i.id into v_item_id from public.inbox_items i
      where i.household_id = v_loan.household_id and i.source_type = 'guided' and i.source_id = p_loan_id;
    end if;
  end if;

  return jsonb_build_object(
    'ok', true, 'paymentId', v_payment_id, 'transactionId', v_tx_id,
    'remainingPrincipal', v_remaining, 'completed', v_completed, 'inboxItemId', v_item_id,
    'amount', v_amount, 'principalPaid', v_principal, 'interestPaid', v_interest,
    'feePaid', 0, 'sourceDelta', -v_amount, 'scheduleEntryId', v_entry.id
  );
end;
$$;
