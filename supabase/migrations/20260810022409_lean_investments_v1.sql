...
declare v_hh uuid; v_oid uuid; v_existing uuid; v_h public.investment_holdings%rowtype; v_add jsonb; v_fee jsonb;
  v_cash_fee numeric(18,0):=0; v_net_q numeric(38,18); v_tx uuid; v_fee_h uuid; v_consume jsonb;
begin
  v_hh := public.investment_active_household();
  select id into v_existing from public.investment_operations where household_id=v_hh and idempotency_key=p_idempotency_key;
  if found then return public.investment_operation_receipt(v_existing,true); end if;
  ...
  if not found then raise exception 'Investment holding not found'; end if;
  perform 1 from public.accounts where id=p_cash_account_id and household_id=v_hh and not is_archived for update;
  ...
  for v_fee in select value from jsonb_array_elements(coalesce(p_fees,'[]'::jsonb)) loop
    ...
    elsif v_fee->>'source'='other_investment' then null;
  ...
  v_add:=public.investment_add_holding(v_hh,p_holding_id,v_net_q,p_executed_value_vnd+v_cash_fee);
  ...
  for v_fee in select value from jsonb_array_elements(coalesce(p_fees,'[]'::jsonb)) loop
    if v_fee->>'source'='cash' then
      ...
      if v_fee->>'source'='other_investment' then v_consume:=public.investment_consume_holding(v_hh,v_fee_h,(v_fee->>'quantity')::numeric); end if;
...
declare v_hh uuid; v_oid uuid; v_existing uuid; v_fee jsonb; v_cash_fee numeric(18,0):=0; v_source_fee numeric(38,18):=0;
  v_total_q numeric(38,18); v_c jsonb; v_tx uuid; v_net numeric(18,0); v_fee_h uuid; v_other jsonb;
begin
  ...
  if found then return public.investment_operation_receipt(v_existing,true); end if;
  perform 1 from public.accounts where id=p_cash_account_id and household_id=v_hh and not is_archived for update; if not found then raise exception 'Cash account not found'; end if;
  for v_fee in select value from jsonb_array_elements(coalesce(p_fees,'[]'::jsonb)) loop
    if v_fee->>'source'='cash' then v_cash_fee:=v_cash_fee+(v_fee->>'amountVnd')::numeric;
    elsif v_fee->>'source'='source_asset' then v_source_fee:=v_source_fee+(v_fee->>'quantity')::numeric;
    elsif v_fee->>'source' <> 'other_investment' then raise exception 'Unsupported sell fee source'; end if;
  end loop;
...
begin
  ...
  for v_fee in select value from jsonb_array_elements(coalesce(p_fees,'[]'::jsonb)) loop
    v_fee_tx:=null;
    if v_fee->>'source'='cash' then
      perform 1 from public.accounts where id=(v_fee->>'cashAccountId')::uuid and household_id=v_hh and not is_archived for update; if not found then raise exception 'Cash account not found'; end if;
      insert into public.transactions(household_id,account_id,type,amount,currency,transaction_date,note,category_id,jar_id,status,idempotency_key,created_by,source)
