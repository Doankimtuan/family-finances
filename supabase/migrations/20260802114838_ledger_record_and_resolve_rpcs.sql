...
begin
  ...
  insert into public.transactions (
    ...
    note, category_id, jar_id, created_by, idempotency_key
  )
  values (
    v_household_id, p_account_id, p_type, p_amount, coalesce(v_currency, 'VND'),
    coalesce(p_transaction_date, (timezone('utc', now()))::date),
    nullif(trim(coalesce(p_note, '')), ''), p_category_id, p_jar_id, v_user_id,
  ...
  if p_type = 'expense' and p_jar_id is null then
    ...
    values (
      v_household_id, 'unmapped_expense', 'transaction', v_tx_id, p_amount,
      coalesce(v_currency, 'VND'), v_title, jsonb_build_object('reason', 'unmapped_expense')
    )
  ...
  then
    ...
    values (
      v_household_id, 'income_suggest', 'transaction', v_tx_id, p_amount,
      coalesce(v_currency, 'VND'), v_title,
      jsonb_build_object(
...
begin
  ...
  set jar_id = p_jar_id, updated_at = now()
  where t.id = v_item.source_id and t.household_id = v_item.household_id;
