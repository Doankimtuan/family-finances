-- Sprint 2 verification fixes (B2 / B3):
-- - Partner-assigned emergency Inbox notifications (BR-13)
-- - OverspendPolicy.BLOCK capacity floor on reallocate (BR-06)

-- ---------------------------------------------------------------------------
-- Partner-targeted inbox assignment
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- reallocate_jar_capacity — BLOCK floor + per-partner emergency notify
-- ---------------------------------------------------------------------------
...
begin
  ...
  -- BR-06: BLOCK refuses capacity moves that would drive source below zero
  ...
  -- BR-13: high-priority Inbox alert for each active partner (not declarer)
    ...
    loop
      ...
      values (
        ...
        jsonb_build_object(
          ...
          'assigned_to_user_id', v_partner.user_id,
          'priority', 'high',
          'channel', 'partner_inbox'
        ),
        v_partner.user_id
      )
      ...
      if v_first_inbox_id is null then
        v_first_inbox_id := v_inbox_id;
      end if;
    end loop;

    -- Solo household: keep an audit ReviewItem visible to the declarer
